<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Gestión de <span class="bg-uts-500 text-gray-800 font-bold me-2 px-2.5 py-0.5 rounded uppercase">Roles</span>
        </h2>
    </x-slot>
    @push('styles')
    <style>
        ...
        #roleModal:target .modal-overlay {
            background-color: rgba(0, 0, 0, 0.5) !important;
        }
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center p-4 border-b">
        <h2 class="text-2xl text-center font-bold text-gray-800">
            Gestión de <span class="bg-uts-500 text-white font-bold me-2 px-2.5 py-0.5 rounded uppercase shadow">Roles</span>
        </h2>
        @can('create_role')
        <button
    ...
    <div id="roleModal" class="fixed z-50 inset-0 overflow-y-auto">
        <div class="modal-overlay absolute inset-0" onclick="closeRoleModal()"></div>
        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center">
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full modal-content relative">
                <button class="modal-close-btn-custom" onclick="closeRoleModal()">
                    &times;
                </button>
                <form id="roleForm" class="p-6 mt-2">
                    @csrf
                    ...
                    <div class="mb-4">
                        <label for="name" class="block font-medium text-sm text-gray-700">
                            <i class="fas fa-tag mr-2 text-gray-500"></i>Nombre del Rol
                        </label>
                        <input
                            type="text"
                            id="name"
    ...
    <script>
        $(document).ready(function() {
            initializeDataTable('#rolesTable', '{{ route("roles.data") }}',
                [{
                        data: 'id',
                        name: 'id'
                    },
                        ...
                        render: function(data, type, row) {
                            return `
                                <div class="flex items-center justify-center container-actions">
                                    @can('edit_role')
                                        <button onclick="editRole(${row.id})" class="btn-action btn-edit hover:bg-uts-800 shadow pointer">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                    @endcan
                                    @can('assign_permissions')
                                        <button onclick="managePermissions(${row.id})" class="btn-action btn-permissions shadow pointer">
                                            <i class="fas fa-key"></i>
                                        </button>
                                    @endcan
                                    @can('delete_role')
                                        <button onclick="deleteRole(${row.id})" class="btn-action btn-delete shadow pointer">
        ...
        function openCreateRoleModal() {
            $('#formTitle').html(`Crear nuevo <span class="bg-uts-500 text-lg text-white font-bold me-2 px-2.5 py-0.5 rounded uppercase shadow">Rol</span>`);
            $('#roleId').val('');
            $('#name').val('');
            $('#description').val('');
            $('#nameError').text('');
